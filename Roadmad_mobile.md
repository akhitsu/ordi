...
- MVP pertama mencakup login, pull master data, transaksi sales offline, dan sync sales saat online.
...
- `features`: auth, outlet selection, menu catalog, cart/checkout, dan sales sync UI.
...
- Pull data dari `GET /api/outlets`, `GET /api/menu-categories`, `GET /api/menu-items`, dan `GET /api/payment-methods`.
...
- Saat checkout, buat `sale.uuid`, `sale_item.uuid`, dan `idempotency_key`.
...
- Retry transaksi memakai payload dan `Idempotency-Key` yang sama.
...
- Tambah payment flow setelah `payments_api.md` final.
...
- Unit test sales payload idempotent: retry transaksi sama menghasilkan payload dan key sama.
...
- Struktur foundation awal dibuat untuk core config, theme, responsive shell, Dio provider, dan Drift schema awal.
- Implementasi repository, DTO mapper, UI auth, katalog menu, checkout, dan sync worker masih tahap berikutnya.

## Assumptions

- Payment push belum masuk MVP karena `payments_api.md` belum final.
...
- Idempotency key untuk transaksi yang sama tidak boleh berubah saat retry.
